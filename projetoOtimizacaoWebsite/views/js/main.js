...
var resizePizzas = function(size) {
  ...
  function determineDx (oldWidth, size, randomPizzaVar) {
    var windowWidth = randomPizzaVar.offsetWidth;
  ...
  function changePizzaSizes(size) {
    ...
    let iteratorPizza  = document.querySelectorAll(".randomPizzaContainer"),
        getOffsetWidth = iteratorPizza[0].offsetWidth,
        randomPizzaVar = document.querySelector("#randomPizzas"),
        dx             = determineDx(getOffsetWidth, size, randomPizzaVar),
        newwidth       = (getOffsetWidth + dx) + 'px';

        console.log("getOffsetWidth", getOffsetWidth);      
        console.log("dx", dx);
        console.log("iteratorPizza[i].offsetWidth", getOffsetWidth);

    for (var i = 0; i < iteratorPizza.length; i++) {
      iteratorPizza[i].style.width = newwidth;
    }
  }
...
function updatePositions() {
  ...
  let scrollVar     = document.body.scrollTop / 1250,
      itemsUp       = document.getElementsByClassName ('mover'),
      itemsUpLength = itemsUp.length;

  for (var i = 0; i < itemsUpLength; i++) {
    var phase = Math.sin((scrollVar) + (i % 5));
    itemsUp[i].style.left = itemsUp[i].basicLeft + 100 * phase + 'px';
  }
...
document.addEventListener('DOMContentLoaded', function() {
  let movingP1 = document.querySelector("#movingPizzas1");
  var cols = 8;
  var s = 256;
  // let countRPC = cols * s;
